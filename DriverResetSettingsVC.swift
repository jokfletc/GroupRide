//
//  DriverResetSettingsVC.swift
//  ResumeProject
//
//  Created by john fletcher on 7/11/16.
...
class DriverResetSettingsVC: UIViewController {
    ...
    func getUsername() {
        ...
        if FIRAuth.auth()?.currentUser != nil{
            ref.child("users").child(userID!).observeSingleEvent(of: .value, with:  { (snapshot) in
                ...
                usernameLabel.frame = CGRect(x: 107, y: 118, width: 198, height: 21)
    ...
    func getGroup() {
        ...
        if FIRAuth.auth()?.currentUser != nil{
            ref.child("users").child(userID!).observeSingleEvent(of: .value, with:  { (snapshot) in
                ...
                groupLabel.frame = CGRect(x: 107, y: 147, width: 198, height: 21)
    ...
    func getEmail() {
        ...
        if FIRAuth.auth()?.currentUser != nil{
            ref.child("users").child(userID!).observeSingleEvent(of: .value, with:  { (snapshot) in
                ...
                emailLabel.frame = CGRect(x: 107, y: 174, width: 198, height: 21)
